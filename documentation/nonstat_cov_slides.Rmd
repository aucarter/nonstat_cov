...
$$
\begin{aligned}
& \vec y|\vec \beta, \vec u, \log(\tau_{\text{noise}}) \sim \mathcal N_N(\mathbf{X} \vec \beta + \mathbf{E}\vec u, \mathbf{I}_N/\tau_{\text{noise}}) \\
& \vec u | \vec \alpha_1, \vec \alpha_2, \vec \alpha_3, \vec \alpha_4 \sim \mathcal N_{nm}(\vec 0, \mathbf{Q}^{-1}), \qquad \vec \beta \sim \mathcal N_p(\vec 0, \mathbf I_p/\tau_{\beta}) \\
& \vec \alpha_i|\tau_i \sim \mathcal N_{kl}(\vec 0, \mathbf Q_{\text{RW2}}^{-1}/\tau_i) \quad \text{for } i = 1,2,3,4 
\end{aligned}
$$
